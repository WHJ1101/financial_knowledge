...
def two_users_with_data():
    ...
    with SessionLocal() as s:
        ...
        inst = Instrument(id=uuid.uuid4(), asset_class="equity", exchange="SZSE",
                          canonical_symbol=sym, display_code=f"SZ{sym}", name="测试标的",
                          market="创业板", provider_ids={}, created_at=datetime.now(UTC),
                          updated_at=datetime.now(UTC))
        s.add(inst)
        s.flush()
        # A 的持仓 + 自选
        s.add(Position(id=uuid.uuid4(), owner_id=uida, instrument_id=inst.id, shares=100, cost=10,
                       created_at=datetime.now(UTC), updated_at=datetime.now(UTC)))
        s.add(WatchlistItem(id=uuid.uuid4(), owner_id=uida, instrument_id=inst.id,
                            created_at=datetime.now(UTC), updated_at=datetime.now(UTC)))
        # A 的私有报告 + 一个共享报告
        s.add(Report(id=f"priv_{uuid.uuid4().hex[:8]}", owner_id=uida, visibility="private",
                     title="A私有", topic="t", type="custom", created_at=datetime.now(UTC),
                     updated_at=datetime.now(UTC)))
        shared_id = f"shared_{uuid.uuid4().hex[:8]}"
        s.add(Report(id=shared_id, owner_id=uida, visibility="shared", title="共享报告", topic="t",
                     type="custom", created_at=datetime.now(UTC), updated_at=datetime.now(UTC)))
        s.commit()
