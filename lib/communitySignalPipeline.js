...
import { resolveLlmUrl, resolveLlmApiKey, resolveLlmModel, extractContent, parseLlmJson } from "./llmClient.js";
import { localDate as formatLocalDate } from "./datetime.js";
...
export async function syncFeishuCommunitySignals({
  ...
  try {
    ...
    const allDays = source.days || [];
    // 文档按天切分，逐天抽取；已入库的天直接跳过，避免重复取全量与重复抽取。
    const pending = allDays.filter((day) => shouldProcessDate(day.date));
    const skippedDates = allDays.filter((day) => !shouldProcessDate(day.date)).map((day) => day.date);
    ...
    const signals = days.flatMap((day) => day.signals);
    const errors = days.map((day) => day.extractionError).filter(Boolean);
    return {
      ok: true,
      skipped: false,
      provider: "feishu",
      ...
      days,
      signals,
      processedDates: pending.map((day) => day.date),
      skippedDates,
      extractionMethod: days.find((day) => day.extractionMethod)?.extractionMethod || (pending.length ? "" : "up-to-date"),
      extractionError: errors.join("; ")
    };
  } catch (error) {
    return {
      ok: false,
...
// 对单天社群文本抽取信号卡。date 为该天的真实日期（来自文档天级标题），
// 信号 id / date / observedAt / expiresAt 都锚定这一天，保证同一天重复同步得到相同结果（幂等去重）。
export async function extractCommunitySignals({
  ...
  const sourceText = String(dayText || "").slice(0, MAX_LLM_SOURCE_CHARS);
  if (!sourceText.trim()) return { method: "empty", signals: [], error: "社群信号源内容为空" };

  const targetDate = date || formatLocalDate(now);
  const context = { date: targetDate, sourceTitle, sourceUrl, provider, now };

  const llm = await callSignalLlm({ sourceText, sourceTitle, sourceUrl, date: targetDate, now, fetchImpl });
  if (llm.ok) {
    try {
      const parsed = parseLlmJson(llm.content);
      const signals = normalizeSignals(parsed.items || parsed.signals || parsed, context);
      if (signals.length) return { method: "llm", signals };
    } catch (error) {
      const signals = fallbackExtractSignals({ sourceText, date: targetDate, sourceTitle, sourceUrl, provider, now });
  ...
  return {
    method: "fallback",
    signals: fallbackExtractSignals({ sourceText, date: targetDate, sourceTitle, sourceUrl, provider, now }),
    error: llm.error || ""
  };
}
...
export function fallbackExtractSignals({
  ...
  }), { date: date || formatLocalDate(now), sourceTitle, sourceUrl, provider, now });
}


function normalizeSignals(values, { date, sourceTitle, sourceUrl, provider, now }) {
  const importedAt = now.toISOString();
  const expiresAt = dayExpiry(date, now);
...
function normalizeSignal(item, context) {
  ...
  const observedAtDefault = context.date ? `${context.date}T00:00:00.000Z` : context.importedAt;
  ...
  const signal = {
    ...
    observedAt: item.observedAt || item.observed_at || observedAtDefault,
...
// 信号有效期锚定到该信号所属的真实日期，而非同步时间——回填旧日期的信号不会被当作"新鲜"信号。
function dayExpiry(date, now) {
  const base = date ? new Date(`${date}T00:00:00.000Z`) : now;
  const anchor = Number.isNaN(base.getTime()) ? now : base;
  return new Date(anchor.getTime() + SIGNAL_TTL_DAYS * 24 * 60 * 60 * 1000).toISOString();
}
...
async function callSignalLlm({ sourceText, sourceTitle, sourceUrl, date, now, fetchImpl }) {
      ...
      },
      {
        ...
        content: JSON.stringify({
          ...
          rules: [
            ...
            "只抽取当前这一天文本内的信号，不要跨天推断或补充其他日期的内容。",
