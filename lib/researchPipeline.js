...
const DEFAULT_EVIDENCE_EXCERPT_LIMIT = 1200;
const STRUCTURED_EVIDENCE_EXCERPT_LIMIT = 5000;
const LLM_EVIDENCE_EXCERPT_LIMIT = 1200;
...
async function collectHttpSources({ topic, type }) {
  ...
  for (const rawUrl of urls.slice(0, 6)) {
    try {
      ...
      const data = await fetchJsonWithTimeout(url, { tolerant: true, timeout: HTTP_TIMEOUT_MS });
    ...
    messages: [
      {
        role: "system",
        content:
          "你是金融与产业研究助理。只输出严格 JSON，不输出 Markdown。结论必须基于给定 evidence，不能编造外部事实。不要生成、改写或序列化证据表格、快讯列表或 HTML；证据区由系统模板统一解析、排序和渲染。"
      },
      {
        role: "user",
        content: JSON.stringify(
          {
            ...
            outputRules: [
              ...
              "highlights/watchList/risks/nextSteps 都必须是字符串数组；每个元素只写一句自然语言，不要写 JSON、表格、键值对或用竖线分隔的数据。",
...
function normalizeRecord({ record, source, fallbackTitle }) {
  ...
  const limit = evidenceExcerptLimit({ title, source, record });
  const excerpt =
    record.summary ||
    record.excerpt ||
    record.description ||
    record.content ||
    compactJson(record, limit);
  ...
  return {
    ...
    excerpt: String(excerpt).slice(0, limit)
  };
}

function evidenceExcerptLimit({ title, source, record }) {
  const label = [
    title,
    source,
    record.type,
  ...
  return /快讯|新闻|资讯|财经|行情|板块|指数|ETF|资金|持仓|成交额|涨跌幅/u.test(label)
    ? STRUCTURED_EVIDENCE_EXCERPT_LIMIT
    : DEFAULT_EVIDENCE_EXCERPT_LIMIT;
}
...
function compactEvidence(item) {
  return {
    ...
    excerpt: String(item.excerpt || "").slice(0, LLM_EVIDENCE_EXCERPT_LIMIT)
  };
}
