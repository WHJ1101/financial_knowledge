...
// DB code -> live code 映射（处理 code 不一致的情况）
const CODE_MAP = { "IXIC.US": "NDX", "SPX.US": "SPX", "HSI.HK": "HSI" };

export function getIndices() {
  const rows = db.prepare("SELECT * FROM market_indices").all();
  const live = getMarketData();
  return rows.map(row => {
    const liveCode = CODE_MAP[row.code];
    const liveItem = live.data.find(d => d.code === liveCode || row.code.includes(d.code));
    return {
